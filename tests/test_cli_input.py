...
from nanobot.cli import input as cli_input
...
def mock_prompt_session():
    ...
    with (
        patch("nanobot.cli.input._PROMPT_SESSION", mock_session),
        patch("nanobot.cli.input.patch_stdout"),
    ):
        yield mock_session
...
async def test_read_interactive_input_async_returns_input(mock_prompt_session):
    ...
    result = await cli_input._read_interactive_input_async()
...
async def test_read_interactive_input_async_handles_eof(mock_prompt_session):
    ...
    with pytest.raises(KeyboardInterrupt):
        await cli_input._read_interactive_input_async()


def test_init_prompt_session_creates_session():
    """Test that _init_prompt_session initializes the global session."""
    # Ensure global is None before test
    cli_input._PROMPT_SESSION = None

    with (
        patch("nanobot.cli.input.PromptSession") as MockSession,  # noqa: N806
        patch("nanobot.cli.input.FileHistory"),
        patch("pathlib.Path.home") as mock_home,
    ):

        mock_home.return_value = MagicMock()

        cli_input._init_prompt_session()

        assert cli_input._PROMPT_SESSION is not None
